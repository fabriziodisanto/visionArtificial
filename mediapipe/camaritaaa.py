...
cv2.namedWindow('6. Ajustes', cv2.WINDOW_NORMAL)
...
cv2.moveWindow('6. Ajustes', 650, 400)

# --- CREAR TRACKBARS PARA BRILLO, CONTRASTE Y SATURACIÓN ---
def nothing(x):
    pass

cv2.createTrackbar('Brillo', '6. Ajustes', 50, 100, nothing)     # 0 - 100
cv2.createTrackbar('Contraste', '6. Ajustes', 50, 100, nothing) # 0 - 100
cv2.createTrackbar('Saturacion', '6. Ajustes', 50, 100, nothing) # 0 - 100
...
while True:
    ...
    # --- APLICAR AJUSTES CON TRACKBARS ---
    brillo = cv2.getTrackbarPos('Brillo', '6. Ajustes') - 50
    contraste = cv2.getTrackbarPos('Contraste', '6. Ajustes') / 50.0
    saturacion = cv2.getTrackbarPos('Saturacion', '6. Ajustes') / 50.0

    # Ajuste de brillo y contraste
    adjusted = cv2.convertScaleAbs(frame, alpha=contraste, beta=brillo)
    ...
    # --- ETAPA 1: Imagen original ajustada
    cv2.imshow('1. Imagen Original', adjusted)

    # --- ETAPA 2: Escala de grises
    gray = cv2.cvtColor(adjusted, cv2.COLOR_BGR2GRAY)
    cv2.imshow('2. Escala de Grises', gray)

    # --- ETAPA 3: Imagen binarizada
    ...
    contour_image = adjusted.copy()
    cv2.drawContours(contour_image, contours, -1, (0, 255, 0), 2)
    cv2.imshow('4. Contornos Detectados', contour_image)

    # --- ETAPA 5: Operaciones morfológicas (apertura + cierre)
    kernel = np.ones((5, 5), np.uint8)
    morphed = cv2.morphologyEx(thresh, cv2.MORPH_OPEN, kernel)
    morphed = cv2.morphologyEx(morphed, cv2.MORPH_CLOSE, kernel)
    cv2.imshow('5. Morfologia', morphed)

    # --- PREDICCIÓN DE FORMAS ---
    result_frame = adjusted.copy()
    if contours:
    ...
    cv2.imshow('6. Ajustes', result_frame)
