...
while True:
    ...
    cleaned_opening = cv2.morphologyEx(bin_img, cv2.MORPH_OPEN, kernel_medium)

    cleaned_closing = cv2.morphologyEx(cleaned_opening, cv2.MORPH_CLOSE, kernel_medium)

    median_filtered = cv2.medianBlur(cleaned_closing, 5)
    cv2.imshow('4. Morfologia', median_filtered)
